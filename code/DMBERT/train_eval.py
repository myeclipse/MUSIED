...
def eval(model, iterator, fname,sent_id2_event):
    ...
    for golden_item in sent_id2_event:
        id = golden_item['id']
        #event_count=len(golden_item['event'])
...
def train(config, model, train_iter, test_iter,sent_id2_event):
    ...
    for epoch in range(config.num_epochs):
        ...
        metric_test, trigger_f1 = eval(model, test_iter, config.save_path+'/'+str(epoch) + '_test',sent_id2_event)
